...
export default class SearchComponentReusable extends LightningElement 
{
    ...
    connectedCallback() {
        ...
        if(this.objectApiName == 'undefined' || this.objectApiName == null || this.objectApiName =='')
        {
            ...
            this.objectApiName=this.label.defaultObj;//no object selected then default object contact will come
            console.log('After label object name update : '+this.objectApiName);
        }
        getMetaConfigs({ objApiName: this.objectApiName })//from getMetaConfig we are passing object api name , here in getametaconfig we are calling metadata( Dynamic_Search_Setting__mdt) and fields 
            .then((results) => {
                this.metResult = results;
                this.errors = undefined;
                console.log('Result in meta : '+this.metResult.Is_Active__c);
                
                if(this.metResult.length !== 0)//if metresults are not empty means
                {
                    this.maxRecordsCount=this.metResult.Max_Records_Too_Display__c;
                    console.log('maxRecordsCount : '+this.maxRecordsCount);
                    this.isActive = this.metResult.Is_Active__c;
                    console.log('isactive : '+this.isActive);
                    this.searchSetName = this.metResult.search_fieldset__c;
                    console.log('isactive : '+ this.searchSetName);
                    this.resultSetName = this.metResult.Result_Fieldset__c;
                    console.log('resultSetName : '+this.resultSetName);
                    this.limitMessage= this.metResult.Error_Message__c;
                    console.log('limitMessage : '+this.limitMessage);
                    this.noDataMessage= this.metResult.No_Data_Message__c;
                    console.log('nodata : '+this.noDataMessage);
                    getFieldsAndRecords({
